...
export function ChatConversation({ withClear = false }: { withClear?: boolean }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="relative mt-2 flex-1 space-y-3 overflow-y-auto rounded-2xl border border-stone-200/80 bg-white p-5">
        {withClear && messages !== null && messages.length > 0 && (
          <button
            onClick={onClear}
            title={t('chat.clearConfirm')}
            className="absolute right-2 top-2 z-10 rounded p-1.5 text-slate-300 hover:bg-red-50 hover:text-red-600"
          >
